...
BEGIN
    ...
    SELECT COUNT(*) INTO v_exists FROM payment WHERE order_id = p_order_id;
    IF v_exists > 0 THEN
        p_error_code := -1;
        p_error_msg  := 'order_id đã tồn tại: ' || p_order_id;
        RETURN;
    END IF;
...
BEGIN
    ...
    -- ====================================================
    -- 1. Kiểm tra payment tồn tại và chưa xử lý (order_status='0')
    -- ====================================================
    BEGIN
    ...
    BEGIN
        ...
        v_num_part := REGEXP_REPLACE(v_last_code, '[^0-9]', '');
        IF v_num_part IS NOT NULL AND LENGTH(TRIM(v_num_part)) > 0 THEN
            v_next_num := TO_NUMBER(v_num_part) + 1;
        END IF;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN v_next_num := 1;
    END;
    v_bill_code := 'HD' || LPAD(TO_CHAR(v_next_num), 3, '0');
