...
public class VpdContextFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        if (auth != null && auth.isAuthenticated() && !"anonymousUser".equals(auth.getPrincipal())) {
            ...
            try {
                ...
                if (account != null) {
                    ...
                    session.doWork(connection -> {
                        ...
                        try (java.sql.CallableStatement cs = connection.prepareCall("{call TRASUA.pkg_vpd_security.set_account_id(?)}")) {
                            cs.setLong(1, account.getId());
                            cs.execute();
                        }

                        // ===== FIX 3C (VPD - Yên): Set context cho Wishlist =====
                        try (java.sql.CallableStatement cs5 = connection.prepareCall("{call TRASUA.pkg_auth_yen.set_account_id(?)}")) {
                        ...
                        try (java.sql.CallableStatement cs3 = connection.prepareCall("{call TRASUA.pkg_branch_sec.set_context(?, ?, ?)}")) {
                        ...
                        try (java.sql.CallableStatement cs2 = connection.prepareCall("{call SA_SESSION.SET_LABEL('BILL_OLS_POL', ?)}")) {
                        ...
                        try (java.sql.CallableStatement cs4 = connection.prepareCall("{call SA_SESSION.SET_LABEL('ACCESS_POLICY', ?)}")) {
            ...
            // Khi chưa đăng nhập (Anonymous), cần Reset Context để dùng chung Connection Pool
            // Nếu không, connection sẽ giữ nhãn của user trước đó, gây lỗi không tìm thấy Account để login
            try {
                org.hibernate.Session session = entityManager.unwrap(org.hibernate.Session.class);
                session.doWork(connection -> {
                    // Xóa VPD Context của Tín (Truyền rỗng để VPD hiểu là Admin/Bypass, cho phép query bảng account)
                    ...
                    try (java.sql.CallableStatement csYen = connection.prepareCall("{call TRASUA.pkg_auth_yen.set_account_id(NULL)}")) {
                        csYen.execute();
                    } catch (Exception ex) {}

                    // Gán OLS Label cao nhất cho ứng dụng để nó có thể query bảng account phục vụ Login
    ...
    private String resolveTinsOlsLabel(Account account) {
        ...
        if ("1".equals(branchId)) branchCompartment = "BR1";
        else if ("2".equals(branchId)) branchCompartment = "BR2";
        else branchCompartment = "BR1,BR2"; // default cho admin hoặc không có chi nhánh
